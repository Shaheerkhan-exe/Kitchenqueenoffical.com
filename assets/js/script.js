...
(function () {
  ...
  const DISHES = [
    ...
    { id: "d4", name: "Lauki Ki Sabzi", cat: "daal", price: 260, serves: 1, img: "Lauki-Sabzi.png", tag: "", desc: "Bottle gourd slow-cooked in tomato, garlic and desi masala — light and homely." },
    ...
    { id: "d7", name: "Kali Daal", cat: "daal", price: 300, was: 400, serves: 1, img: "kali daal.png", tag: "", desc: "Daal maash simmered overnight on low flame with butter and cream." },
    { id: "d8", name: "Bhindi Sabzi", cat: "daal", price: 320, serves: 2, img: "bhindi sabzi.png", tag: "", desc: "Crisp okra bhujia with onion, tomato and hand-crushed spice — never slimy." },
    ...
    /* --- BEEF & MUTTON --- */
    { id: "d12", name: "Beef Bhuna Ghosht", cat: "beef", price: 600, was: 899, serves: 2, img: "bhuna gohst.png", tag: "BESTSELLER", hot: true, desc: "Beef bhunoed for an hour until the masala clings to every piece. No gravy, all flavour." },
    { id: "d13", name: "Chicken Qourma", cat: "Chicken", price: 450, was: 850, serves: 2, img: "chicken qourma.png", tag: "NEW", hot: true, desc: "Slow-cooked chicken in a silky fried-onion and yogurt qourma — festive, homely and made for roti." },
    { id: "d14", name: "Arvi Ghosht", cat: "beef", price: 450, serves: 2, img: "Arvi.png", tag: "", desc: "Tender mutton and arvi in a rich brown gravy — a Sunday classic." },
    { id: "d15", name: "Aalu Ghosht", cat: "beef", price: 450, serves: 2, img: "Aloo gohst.png", tag: "", desc: "Mutton and potato simmered together in traditional desi masala." },
    { id: "d16", name: "Qeema Aalu", cat: "beef", price: 450, serves: 2, img: "keema-Aaloo.png", tag: "", desc: "Minced beef bhuna with soft potato cubes, whole spice and green chilli." },

    /* --- HALEEM --- */
    { id: "d17", name: "Beef Haleem", cat: "haleem", price: 480, was: 460, serves: 2, img: "beef haleem.png", tag: "BESTSELLER", hot: true, desc: "Seven-hour stone-ground wheat and lentils with shredded beef and crisp onion." },
    { id: "d18", name: "Chicken Haleem", cat: "haleem", price: 480, serves: 2, img: "chicken haleem.png", tag: "", hot: true, desc: "Lighter haleem with shredded chicken, ginger julienne and lemon on the side." },
    ...
    { id: "d23", name: "Shami Kabab (6 pcs)", cat: "sides", price: 500, serves: 2, img: "shami.png", tag: "", desc: "Melt-in-mouth beef and chana daal patties, pan-seared in ghee." },
    { id: "d24", name: "Dahi Phulki with Tarka", cat: "sides", price: 280, serves: 2, img: "dahi phulki.png", tag: "", desc: "Soft phulki in whisked yoghurt with a red chilli and curry-leaf tarka." },
    { id: "d25", name: "Sweet Dahi Phulki", cat: "sides", price: 280, serves: 2, img: "Sweet phulki.png", tag: "", desc: "Sweetened yoghurt, soft phulki and mint — cooling and mildly sweet." },
    { id: "d26", name: "Baingan Ka Raita", cat: "sides", price: 280, serves: 2, img: "baingan raita.png", tag: "", desc: "Roasted aubergine whisked into yoghurt with zeera and black salt." },
    ...
    { id: "d32", name: " Home Made chapati(4 pcs)", cat: "sides", price: 120, serves: 2, img: "chapati.png", tag: "", desc: "Soft homemade whole-wheat chapati, cooked on tawa and served hot." },

    /* --- BEVERAGES --- */
    { id: "d28", name: "Cold Drinks", cat: "drinks", price: 150, serves: 1, img: "pepsi.avif", tag: "", desc: "345ml chilled bottle — your choice of flavour." },
    { id: "d29", name: "Lassi", cat: "drinks", price: 250, serves: 1, img: "lassi.png", tag: "", desc: "Hand-churned yoghurt lassi — sweet or namkeen, served ice cold." },
  ...
  const EASYPAISA = { name: "Muhammad Sami Khan", number: "0345-2269366" };
  ...
  function renderShell() {
    ...
    if (document.body.classList.contains("kq-checkout-page")) {
    ...
    hours.innerHTML = `<div class="kq-marquee" aria-hidden="false">
        <span>For any queries, our working hours 12PM to 12AM — call us at (033) 62465676 or WhatsApp 92-3362465676</span>
        <span>For any queries, our working hours 12PM to 12AM — call us at (033) 62465676 or WhatsApp 92-3362465676</span>
      </div>`;
    }
  ...
  function openCustomizer(id, editIndex) {
    ...
    box.className = "kq-modal-box kq-cust-box";
    const spiceBlock = cfg.spice ? `
          <div class="kq-opt-group">
            <div class="kq-opt-label">SPICE LEVEL</div>
            ${ADDONS.spice.map((o, i) => optRow("radio", "spice", o, existing ? existing.spice === o.label : i === 1)).join("")}
          </div>` : "";
    const extraBlock = cfg.extras ? `
          <div class="kq-opt-group">
            <div class="kq-opt-label">ADD EXTRAS <span class="kq-opt-badge">OPTIONAL</span></div>
            ${ADDONS.extras.map((o) => optRow("checkbox", "extra", o, existing && existing.extras ? existing.extras.some((e) => e.label === o.label) : false)).join("")}
          </div>` : "";
    box.innerHTML = `
      <button class="kq-x-btn kq-cust-close" id="kqCustClose" type="button" aria-label="Close"><i class="fa-solid fa-xmark"></i></button>
      <div class="kq-cust-grid${!cfg.spice && !cfg.extras ? " desc-only" : ""}">
    ...
    const closeBtn = box.querySelector("#kqCustClose");
    if (closeBtn) closeBtn.addEventListener("click", () => KQ.closeAll());
      ...
      ${state.cart.map((i, idx) => `
        <div class="kq-cart-item">
          ...
          <div class="kq-ci-info">
            ...
            <p class="kq-ci-meta">${[i.portion, i.spice].filter(Boolean).map(esc).join(" · ")}${i.extras && i.extras.length ? "<br>+ " + i.extras.map((e) => esc(e.label)).join(", ") : ""}${i.note ? "<br><em>“" + esc(i.note) + "”</em>" : ""}</p>
            <div class="kq-ci-row">
  ...
  const pages = {
    index() {
      ...
      if (cats) {
        const pills = [{ id: "all", label: "All", icon: "fa-utensils" }, ...CATEGORIES];
        cats.innerHTML = pills.map((c) => `<button class="kq-cat-pill${c.id === "all" ? " active" : ""}" data-cat="${c.id}"><i class="fa-solid ${c.icon}"></i> ${c.label}</button>`).join("");
    ...
    "meal-builder"() {
      const STEPS = [
        { key: "rice", name: "Rice / Bread", opts: [["chicken biryani", 350, "chicken biryani.png"], ["sweet Dahi Phulki", 280, "Sweet phulki.png"], ["Tandoori Naan","4 pieces", 200, "naan.jpg"], ["Plain white rice", 250, "plain-white-rice.png"]] },
        { key: "main", name: "Main Dish", opts: [["Chicken Karahi", 850, "karahi.jpg"], ["Beef Haleem", 480, "beef haleem.png"], ["Aloo Qeema", 450, "keema-Aaloo.png"], ["Aloo Gosht", 450, "Aloo gohst.jpg"]] },
        { key: "side", name: "Side", opts: [["Shami Kabab","6 Pieces", 500, "shami.png"], ["Mint Raita", 90, "raita.jpg"], ["Green Salad", 70, "raita.jpg"], ["Daal Chawal", 350, "Daal Chawal.jpg"]] },
        { key: "sweet", name: "Dessert & Drink", opts: [["Desi Kheer", 180, "kheer.png"], ["Cold Drink", 150, "pepsi.avif"], ["Kheer + Drink", 330, "kheer.png"], ["Chicken Qourma", 450, "chicken qourma.png"]] }
      ];
    ...
    checkout() {
      ...
      const readSlip = (file, cb) => {
        if (!file) return cb("");
        if (!/^image\//.test(file.type)) return toast("Please upload a payment screenshot image", "fa-triangle-exclamation");
        const img = new Image();
        ...
        img.onerror = () => { URL.revokeObjectURL(url); toast("Could not read screenshot", "fa-triangle-exclamation"); };
        img.src = url;
      };

      const slipInput = document.getElementById("epSlip");
      const slipPreview = document.getElementById("epSlipPreview");
      if (slipInput) {
          ...
          readSlip(f, (data) => {
            ...
            if (slipPreview) {
              if (data) {
                ...
                slipPreview.querySelector("img").src = data;
              } else {
                slipPreview.hidden = true;
              }
            }
            slipInput.closest(".kq-ep-upload")?.classList.toggle("has-file", !!data);
          });
        });
      }
      const copyBtn = document.getElementById("epCopy");
      if (copyBtn) {
        copyBtn.addEventListener("click", async () => {
          try {
            await navigator.clipboard.writeText(EASYPAISA.number.replace(/-/g, ""));
            toast("EasyPaisa number copied", "fa-copy", "success");
          } catch (err) {
            toast(EASYPAISA.number, "fa-mobile-screen");
          }
        });
      }
      ...
      const draw = () => {
        const t = totals();
        const gst = Math.round(t.sub * 0.15 / 1.15);
        wrap.innerHTML = state.cart.length
          ? state.cart.map((i) => `<div class="bp-sum-item">
                <img src="${IMG + (i.img || "Chicken Biryani.png")}" alt="">
                <div>
                  <b>${i.qty}x ${esc(i.name)}</b>
                  <small>${esc([i.portion, i.spice].filter(Boolean).join(", ") || "Home-cooked")}</small>
                </div>
                <span class="price">${money(i.unit * i.qty)}</span>
              </div>`).join("") +
            `<div class="bp-bill">
              <div class="bp-bill-row"><span>Items Total</span><span>${money(t.sub)}</span></div>
              ${t.discount ? `<div class="bp-bill-row"><span>Discount</span><span>- ${money(t.discount)}</span></div>` : ""}
              <div class="bp-bill-row"><span>Incl. GST (15%)</span><span>${money(gst)}</span></div>
              <div class="bp-bill-row"><span>Delivery Fee</span><span>${t.delivery ? money(t.delivery) : "Free"}</span></div>
              <div class="bp-bill-row total"><span>TOTAL PAYABLE</span><span>${money(t.total)}</span></div>
            </div>`
          : `<p style="color:#888;font-size:.85rem">Your basket is empty. <a href="index.html" style="color:#e0a800;font-weight:700">Browse the menu →</a></p>`;
      };
      draw();
      document.addEventListener("kq:cart", draw);

      const epBox = document.getElementById("kqEasyPaisaFields");
      const payBtns = document.querySelectorAll(".kq-pay-opt, .bp-pay");
      payBtns.forEach((p) => p.addEventListener("click", () => {
      ...
      form.addEventListener("submit", (e) => {
        ...
        if (selectedPay === "easypaisa") {
          if (!epSlipData) {
            document.querySelector(".kq-ep-upload")?.classList.add("invalid");
            return toast("Please upload your EasyPaisa payment screenshot", "fa-triangle-exclamation");
          }
        }
        placeOrder();
      });
      ...
      function placeOrder() {
        ...
        lastOrder = {
          ...
          epName: EASYPAISA.name, epNumber: EASYPAISA.number,
          slip: epSlipData || "",
          status: pending ? "pending" : "confirmed",
          total: t.total, items: [...state.cart], time: Date.now()
        };
        try {
          const slim = Object.assign({}, lastOrder);
          if (slim.slip && slim.slip.length > 400000) slim.slip = slim.slip.slice(0, 400000);
        ...
        setTimeout(() => {
          ...
          badge.querySelector("span").innerHTML = pending
            ? `AWAITING APPROVAL · <b>${orderNo}</b>`
            : `ORDER CONFIRMED · <b>${orderNo}</b>`;
          toast(pending ? "Screenshot submitted — waiting for admin approval" : "Order placed successfully!", pending ? "fa-hourglass-half" : "fa-check", "success");
  ...
  function validate(form) {
    ...
    form.querySelectorAll("[data-required]").forEach((input) => {
      const field = input.closest(".kq-field, .bp-input, .bp-card") || input;
      ...
      input.style.borderColor = bad ? "#e03131" : "";
      if (bad) ok = false;
    });
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        if (location.hash === "#preview") {
          document.querySelectorAll(".kq-modal,.kq-overlay").forEach((el) => el.classList.remove("show"));
          document.body.style.overflow = "";
          return;
        }
